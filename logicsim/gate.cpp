...
{
    wireInput(0,in); //takes in 1 input, has one output
}

Event* NotGate::update(uint64_t current_time) { //I used the same code as Or2Gate and adjusted checks for NotGate.
	char state = '0';
  Event* e = nullptr;
	auto w = m_inputs[0];
	char in = w->getState();
